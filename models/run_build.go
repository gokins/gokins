...
type RunBuild struct {
	...
	TimeStamp         time.Time `xorm:"comment('执行时长') DATETIME" json:"timeStamp"`
	Title             string    `xorm:"comment('标题') VARCHAR(255)" json:"title"`
	Message           string    `xorm:"comment('构建信息') VARCHAR(255)" json:"message"`
